 ...
 *
 * @brief: Read identification and bit-wise majority voting for oligo pool data storage
 *
 *************************************/
#include <cstdlib>
#include <cstdio>
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 9)
    {
        fprintf(stderr, "Usage: "
                        ...
                        "[Thread num] [Threshold] [Mode] [Refer file]\n",
                argv[0]);
        return 1;
    }
    ...
    sscanf(argv[6], "%d", &enhanced_mode);
    sscanf(argv[7], "%s", refer_file);
    sscanf(argv[8], "%d", &bwa_idx_flag);
    ...
    for (int ni = 0; ni < n_thread; ++ni)
    {
        ...
        (worker_hdlr + ni)->enhanced_mode = enhanced_mode;
